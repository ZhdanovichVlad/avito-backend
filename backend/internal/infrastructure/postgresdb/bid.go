...
import (
	"avitoTest/backend/internal/domain"
	"database/sql"
	"errors"
	"fmt"
	"log"
...
`
	...
	createType = `
	CREATE TYPE author_types AS ENUM ('Organization', 'User');
`
	_, err = s.db.Exec(createType)
	if err != nil {
...
// CreateNewBid Method of creating a new bid in the database
func (s *Storage) CreateNewBid(bid domain.Bid) (Id string, err error) {
...
func (s *Storage) GetBids(bids *[]domain.Bid, limit, offset int, username, companyID, tenderID string, bidSearchType int) error {
...
// GetBid method to retrieve a bid.
func (s *Storage) GetBid(bid *domain.Bid, bidID string) (successfulRequest bool, err error) {
...
func (s *Storage) UpdateBid(bid domain.Bid) error {
