...
// An interface that verifies the existence of the organization and employees
...
// FullExist function for checking the existence of a user, company and whether the user is a representative of the company
func (s *ShardeDomain) FullExist(creatorUsername, organizationId string) (httpCode int, err error) {
	...
	сhecking, err := s.infrastructureDB.CheckOrganizationExists(organizationId)
	if err != nil {
...
func (s *ShardeDomain) UserNameExist(creatorUsername string) (httpCode int, err error) {
	const op = "application.shared"
...
func (s *ShardeDomain) LimitAndOffsetCheck(limit, offset string) (limitInt int, offsetInt int, err error) {
...
func (s *ShardeDomain) IsTenderServiceTypeIncorrect(serviceType string) bool {
